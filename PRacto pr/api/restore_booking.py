...
class handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            ...
            get_req = urllib.request.Request(f"{base}/deleted_bookings/{booking_id}", method='GET')
            with urllib.request.urlopen(get_req) as res:
                booking_doc = json.loads(res.read().decode('utf-8'))

            # 2. Extract fields and remove deletion metadata
            ...
            # 3. Write back to active bookings collection
            put_url = f"{base}/bookings/{booking_id}"
            ...
            # 4. Delete from deleted_bookings collection
            del_req = urllib.request.Request(f"{base}/deleted_bookings/{booking_id}", method='DELETE')
            with urllib.request.urlopen(del_req) as res:
                res.read()

            self._respond(200, {'status': 'success', 'message': 'Booking restored successfully.'})
